...
class GeminiClient(LLMClient):
    ...
    def generate_evaluation(self, explanation, topics, prompt: str, links_dict: dict[str,str]) -> EvalGen:
        ...
        res = self.client.models.generate_content(
            model=self._MODEL,
            contents=[user_input, 
                      Part.from_uri(file_uri=link,mime_type=mime)
                      for link,mime in links_dict.items()],
            config=GenerateContentConfig(
                system_instruction=prompt,
                response_mime_type="application/json",
